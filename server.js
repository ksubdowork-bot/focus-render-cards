  ...
  try {
    ...
    const messages = [
      { role: "system", content: 
        ...
        `너는 다음 페르소나처럼 답변하는데, 페르소나에 있는 내용을 바탕으로, 구체적으로 대답하고, 오프라인 체험 및 모바일기기 사용에 관해서는 꼭 포함시켜, 마지막에는 대답에 따른 삼성갤럭시 오프라인 팝업에 쓸 수 있는 인사이트를 3개 추출해서 간결하게 추가해줘. 체험 요소로는 카메라 촬영 (셀피, 저조도) AI를 활용한 체험등이 있을 수 있어 갤럭시 25의 기능을 먼저 숙지하고 대답과 연관지어서 작성
...
app.post("/chat/group", async (req, res) => {
  try {
    ...
    const sys =
      ...
      체험 요소로는 카메라 촬영 (셀피, 저조도) AI를 활용한 체험등이 있을 수 있어 갤럭시 25의 기능을 먼저 숙지하고 대답과 연관지어서 작성해줘
      
      
      \n` +
      `각 발언은 반드시 "이름: 내용" 형식으로 한 줄씩 출력해. 마지막엔 '인사이트:' 한 줄을 붙여.\n` +
